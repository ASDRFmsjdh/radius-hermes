...
validate_platforms() {
  ...
  if [[ "$count" -lt 1 ]]; then
    echo "[bootstrap] ERROR: Configure at least one platform: Telegram, Bale, Discord, or Slack." >&2
    exit 1
  fi
}
...
for key in \
  ...
  BALE_BOT_TOKEN BALE_ALLOWED_USERS BALE_ALLOW_ALL_USERS BALE_HOME_CHANNEL BALE_HOME_CHANNEL_NAME \
